...
# dim_cc = 300
# layer_dims_enc = [dim_cc - 1,150,50,10]
# latent_dims = 2
# layer_dims_dec = [latent_dims,10,50,150,dim_cc - 1]
# # Pseudo Variables(Ignore)
# flag_cond = 0
# num_cond = 0
...
for ld in latent_dims_list:
	...
	for epoch in range(num_epochs):
		...
		for iteration, (label, pitch, velocity, x) in enumerate(data_loader_train):
			...
			# x = x[:,1:]
			# xinp = torch.cat((x.float(),pitch.view(-1,1).float()),dim = -1)
